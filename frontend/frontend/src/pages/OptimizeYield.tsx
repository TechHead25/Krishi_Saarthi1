...
const OptimizeYield = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${BACKEND_URL}/optimize_yield`, {
        method: 'POST',
  ...
  return (
    <div className="space-y-6 animate-slide-up">
      <FeatureCard
        title={t('optimize_header')}
        subtitle={t('optimize_sub')}
      ...
      {result && (
        <div className="space-y-6">
          <div className="grid md:grid-cols-3 gap-4">
            <ResultCard
              title={t('optimized_yield')}
              value={`${result.optimized_yield_t_per_ha} t/ha`}
              icon={<Zap className="w-5 h-5" />}
            />
            
            <ResultCard
              title={t('best_fertilizer')}
              value={`${result.best_fertilizer_kg_per_ha} kg/ha`}
              variant="warning"
            />
